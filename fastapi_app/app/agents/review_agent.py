...
class ReviewState(TypedDict):
    ...
    # Input parameters
    username: Annotated[str, lambda x, y: y] # Changed from student_surname
    lecture_number: Annotated[int, lambda x, y: y]
    task: Annotated[Optional[str], lambda x, y: y]
    # github_nickname is removed
...
class LectureReviewState(TypedDict):
    ...
    # Input parameters
    lecture_number: Annotated[int, lambda x, y: y]
    students: Annotated[Optional[List[str]], lambda x, y: y] # This now refers to usernames
...
class HomeworkReviewAgent:
    ...
    def _validate_input(self, state: ReviewState) -> ReviewState:
        ...
        try:
            ...
            logger.info(f"Input validation passed for {state['username']}")
            return state
    ...
    def _get_student_code(self, state: ReviewState) -> ReviewState:
        """Get student code from their GitHub repository."""
        try:
            username = state["username"]
            code_content = {}

            # Fetch from GitHub is now the only method
            repo_path = self.repo_service.get_homework_from_github(
                username,
                state["lecture_number"]
            )
            tasks = [d.name for d in repo_path.iterdir() if d.is_dir() and d.name.startswith("task_")]

            if state.get("task"):
                tasks = [f"task_{state['task']}"]

            for task_folder in tasks:
                task_path = repo_path / task_folder
                task_code = self.repo_service.read_code_from_path(task_path)
                # Prepend the task folder to the file paths to keep them unique
                for fname, code in task_code.items():
                    code_content[f"{task_folder}/{fname}"] = code
            ...
            logger.info(f"Retrieved code for {state['username']}")
            return state
    ...
    def _analyze_code(self, state: ReviewState) -> ReviewState:
        ...
        try:
            ...
            logger.info(f"Code analysis completed for {state['username']}")
            return state
    ...
    def _review_code(self, state: ReviewState) -> ReviewState:
        ...
        try:
            all_code_content = state["code_content"]
            review_results = []

            # Group code files by task
            tasks_with_code = defaultdict(dict)
            for file_path, content in all_code_content.items():
                # Assuming path is like 'task_1/calculator.py'
            ...
            logger.info(f"Code review completed for {state['username']}")
            return state
    ...
    def _calculate_score(self, state: ReviewState) -> ReviewState:
        ...
        try:
            ...
            logger.info(f"Score calculation completed for {state['username']}")
            return state
    ...
    def _save_results(self, state: ReviewState) -> ReviewState:
        ...
        try:
            ...
            logger.info(f"Results saved for {state['username']}")
            return state
    ...
    async def review_student(
        ...
        """
        Review a student's homework submission from GitHub.
        """
